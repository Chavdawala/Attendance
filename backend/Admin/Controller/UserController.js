...
const getUsersByDepartment = async (req, res) => {
  try {
    ...
    const departmentData = await UserDetails.findOne({ department }); // Find the department
    if (!departmentData) {
      return res.status(404).json({ error: 'Department not found' });
    }
    res.status(200).json(departmentData.users); // Return the users in the department
  } catch (error) {
...
const getUserById = async (req, res) => {
  try {
    ...
    // Find the user by ID within the users array
